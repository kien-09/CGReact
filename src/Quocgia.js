...
export function Quocgia() {
    ...
    let findByPopulation = () => {
        let from = 100000;
        let to = 1000000;
        let newList = list.filter((item) => {
            let population = item.population;
            return population >= from && population <= to;
        });
        setList(newList);
    }
    return (
        <>
            ...
            <button onClick={findByPopulation}>Danh sách các nước dân số 100000 - 1000000</button>
